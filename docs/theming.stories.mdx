...
# What is a theme
The whole idea beind UI library is to be theme agnostic, so each component does not use a css class or a tag directly into the template but one that is configured in an injected object.
