...
You can play around with that functionallity [here](/story/ui-bootstrap-dropdown--edge)
...
Note that the Modal uses [UiMenu](?path=/docs/ui-bootstrap-menu-normal&args=#menu) and [UiButton](?path=/docs/ui-bootstrap-button--primary&args=#buttons) components internally, so when providing a theme you should provide for them as well.