...
# Toast message
<Canvas>
  <Story id="ui-toast--normal" />
</Canvas>

# Positioned
<Canvas>
  <Story id="ui-toast--positioned" />
</Canvas>

# Timer
<Canvas>
  <Story id="ui-toast--timer" />
</Canvas>

# Non dismissible
<Canvas>
  <Story id="ui-toast--non-dismissable" />
</Canvas>

# Without header
<Canvas>
  <Story id="ui-toast--no-header" />
</Canvas>
...
Note that the Toast uses [UiTransition](?path=/docs/ui-transition-default&args=#transitions) and [UiButtonclose](?path=/docs/ui-closebutton--default&args=#close-button) components internally, so when providing a theme you should provide for them as well.